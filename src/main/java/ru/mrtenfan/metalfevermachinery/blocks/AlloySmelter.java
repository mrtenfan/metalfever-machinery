...
public class AlloySmelter extends BlockContainer {
	...
	public AlloySmelter(boolean work) {
		super(Material.iron);
		if(!work)
		...
		this.setBlockName("alloy_smelter");
		this.isActive = work;
	}

	@Override
	public Item getItemDropped(int p_149650_1_, Random p_149650_2_, int p_149650_3_) {
		return Item.getItemFromBlock(MFMBlocks.alloy_smelter);
	}
	...
	public void registerBlockIcons(IIconRegister par1IconRegister) {
		...
		BlockIconFront = par1IconRegister.registerIcon(this.isActive ? texPath + "alloy_smelter_front_active" : texPath + "alloy_smelter_front");
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ) {
		if (world.isRemote) {
			...
			if (entity != null) {
				FMLNetworkHandler.openGui(player, MetalFeverMachinery.instance, MFMBlocks.GUI_ID_alloy_smelter, world, x, y, z);
			}
			return true;
		} else {
			return false;
	...
	public static void updateBlockState(boolean isAlloying, World world, int xCoord, int yCoord, int zCoord) {
		...
		if (isAlloying) 
			world.setBlock(xCoord, yCoord, zCoord, MFMBlocks.alloy_smelter_active);
		else 
			world.setBlock(xCoord, yCoord, zCoord, MFMBlocks.alloy_smelter);
