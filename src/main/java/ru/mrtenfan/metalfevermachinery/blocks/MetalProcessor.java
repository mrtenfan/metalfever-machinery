...
public class MetalProcessor extends BlockContainer {
	...
	public MetalProcessor(boolean work) {
		super(Material.iron);
		if(!work)
		...
		this.setBlockName("metal_processor");
		this.isActive = work;
	}

	@Override
	public TileEntity createNewTileEntity(World p_149915_1_, int p_149915_2_) {
		return new TileEntityMetalProcessor();
	}
	...
	public void registerBlockIcons(IIconRegister par1IconRegister) {
		...
		BlockIconFront = par1IconRegister.registerIcon(this.isActive ? texPath + "metal_processor_front_active" : texPath + "metal_processor_front");
...
//		System.out.println(side);
		if (world.isRemote) {
			...
			TileEntityMetalProcessor entity = (TileEntityMetalProcessor) world.getTileEntity(x, y, z);
			if (entity != null) {
				FMLNetworkHandler.openGui(player, MetalFeverMachinery.instance, MFMBlocks.GUI_ID_electrical_furnace, world, x, y, z);
			}
			return true;
		} else {
			return false;
	...
	public static void updateBlockState(boolean isAlloying, World world, int xCoord, int yCoord, int zCoord) {
		...
		if (isAlloying) 
			world.setBlock(xCoord, yCoord, zCoord, MFMBlocks.metal_processor_active);
		else 
			world.setBlock(xCoord, yCoord, zCoord, MFMBlocks.metal_processor);
	...
	public Item getItem(World world, int x, int y, int z) {
		return Item.getItemFromBlock(MFMBlocks.metal_processor);
	}
